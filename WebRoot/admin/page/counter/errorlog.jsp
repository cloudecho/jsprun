...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="guide">
	<tr><td><a href="#" onclick="parent.menu.location='admincp.jsp?action=menu'; parent.main.location='admincp.jsp?action=home';return false;"><bean:message key="header_system" /></a>&nbsp;&raquo;&nbsp;<bean:message key="menu_log_error" /></td></tr>
</table>
<br />
			<table width="100%" border="0" cellpadding="0" cellspacing="0"
				...
				<tr class="header">
					<td colspan="3">
						<bean:message key="menu_log_error" />
					</td>
				</tr>
				<form method="post" action="admincp.jsp?action=errorlog">
						...
						<c:if test="${logpage.totalSize > lpp}">
							<div class="pages">
								...
								<c:if test="${logpage.totalPage>10 && logpage.currentPage>=4}">
									<a href="admincp.jsp?action=errorlog&lpp=${lpp}&amp;page=1"
										class="first">1 ...</a>
								</c:if>
								
								<c:if test="${logpage.currentPage != logpage.prePage}">
									<a href="admincp.jsp?action=errorlog&lpp=${lpp}&amp;page=${logpage.prePage}" class="prev">&lsaquo;&lsaquo;</a>
								</c:if>
								<c:choose>
									<c:when
										...
										<c:forEach var="num" begin="${logpage.currentPage-2}"
											...
											<c:choose>
												...
												<c:otherwise>
													<a href="admincp.jsp?action=errorlog&lpp=${lpp}&amp;page=${num}">${num}</a>
												</c:otherwise>
											</c:choose>
										</c:forEach>
									</c:when>
								...
								<c:if test="${logpage.currentPage != logpage.nextPage}">
									<a
										href="admincp.jsp?action=errorlog&lpp=${lpp}&amp;page=${logpage.nextPage}"
										class="next">&rsaquo;&rsaquo;</a>
								</c:if>
								
								<c:if test="${logpage.totalPage>10 && (logpage.totalPage-logpage.currentPage)>7}">
									<a
									href="admincp.jsp?action=errorlog&lpp=${lpp}&amp;page=${logpage.totalPage}"
								...
								<c:if test="${logpage.totalPage>10}">
									...
									<input type="text" name="custompage" size="3"
										onkeydown="if(event.keyCode==13) {window.location='admincp.jsp?action=errorlog&lpp=${lpp}&amp;page='+this.value; return false;}" />
								</kbd>
								</c:if>
						</c:if>
			
				<c:forEach items="${errorloglist}" var="errorlog">
							<tr align="center">
					<td class="altbg1">
						${errorlog.type}
					</td>
					<td class="altbg2">
						<a href="space.jsp?action=viewpro&username=<jrun:encoding value="${errorlog.username}"/>" target="_blank">${errorlog.username}</a>
						<br />
					</td>
					<td class="altbg1">
						${errorlog.datetime}
