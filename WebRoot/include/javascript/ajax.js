...
function evalscript(s) {
	...
	var arr = new Array();
	var p = /<script (?!src)[^\>]*?( reload=\"1\")?>([^\x00]+?)<\/script>/ig;
	while(arr = p.exec(s)) appendscript('', arr[2], arr[1]);
	p = /<script[^\>]*?src=\"([^\>]*?)\"[^\>]*?(reload=\"1\")?(?:charset=\"([\w\-]+?)\")?><\/script>/ig;
	while(arr = p.exec(s)) appendscript(arr[1], '', arr[2], arr[3],arr[4]);
	return s;
}
function appendscript(src, text, reload, charset,callBack) {
	...
	scriptNode.id = id;
	charset?scriptNode.charset = charset:null;
	if(callBack){
		scriptNode.onload=scriptNode.onreadystatechange=function(){
			if(scriptNode.readyState  && scriptNode.readyState != 'loaded' && scriptNode.readyState != 'complete') return ;   
...
function ajaxposts(formid, showid, waitid,loading, display) {
	var x = new Ajax();
	var waitid = typeof waitid == 'undefined' || waitid === null ? waitid : (waitid !== '' ? waitid : '');
	x.setLoading(loading);
	x.setWaitId(waitid);
	x.display = typeof display == 'undefined' || display == null ? '' : display;
	var s = "";
	var elelist = $(formid).elements;
  	for(var i=0;i<elelist.length;i++){  
  	var obj = null;
  		if(is_ie){
  			obj=elelist.elements[i];
  		}else{
  			obj = elelist[i];
  		}
  		if(obj.type=='checkbox'){
  			...
  			s+=obj.name+"="+obj.value.replace(/&nbsp;/g,'').replace(/&/g,'')+"&";
  		}
    }
    s = s.substring(0,s.length-1);
    x.post($(formid).action,s,function(m, x) {
		if(m.indexOf('error:')!=-1){
			...
			if(m.indexOf('show:')!=-1){
				m = m.substring(5);
				ajaxinnerhtml($(showid), m);
				setMenuPosition($(showid).ctrlid, 0);
