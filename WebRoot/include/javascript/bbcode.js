...
function bbcode2html(str) {
	...
	if(!fetchCheckbox('bbcodeoff') && allowbbcode) {
		str= str.replace(/\s*\[code(=(.*?))?\]([\s\S]+?)\[\/code\]\s*/ig, function($1, $2,$3,$4) {return parsecode($2,$4);});
	}
	...
	if(!fetchCheckbox('smileyoff') && allowsmilies) {
		for(id in smilies) {
			if (smilies[id]['code']) {
				...
				str = str.replace(re, '<img src="./images/smilies/' + smilies[id]['url'] + '" border="0" smilieid="' + id + '"/>');
			}
		}
	}
	if(!fetchCheckbox('bbcodeoff') && allowbbcode) {
	...
	if(!fetchCheckbox('bbcodeoff')) {
		if(allowimgcode) {
			str = str.replace(/\[localimg=(\d{1,4}),(\d{1,4})\](\d+)\[\/localimg\]/ig, function ($1, $2, $3, $4) {if($('attach_' + $4)) {var src = $('attachlocal_' + $4).value; if(src != '') return '<img style="filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod=\'scale\',src=\'' + src + '\');width:' + $2 + ';height=' + $3 + '" src=\'images/common/none.gif\' border="0" aid="attach_' + $4 + '" alt="" />';}});
...
function parsenewcode(text1,text2,text3,text4,text5) {
	text2 = isUndefined(text2)?'':text2;
	var text = '<p>'+text1+'[code'+text2+']'+text4+'[/code]'+text5+'</p>';
	text = recursion('p', text, 'dpstag');
	return text;
}
...
function html2bbcode(str) {
	...
	if(forumallowhtml || (allowhtml && fetchCheckbox('htmlon')) || trim(str) == '') {
		...
		str = str.replace(/<p>([\s\S]*)\[code(=(.*?))?\]([\s\S]+?)\[\/code\]([\s\S]*)<\/p>/ig, function($1, $2,$3,$4,$5,$6){return parsenewcode($2,$3,$4,$5,$6);});
		str = parsemozcode(str, 'bbcode');
		return str;
	}
...
function htmlspecialcharscode(str) {
	str = preg_replace([(is_mac && is_ie ? '&' : '&(?!#[0-9]+;)'), '<', '>', '"'], ['&amp;', '&lt;', '&gt;', '&quot;'], str);
	return preg_replace(['&amp;lt;','&amp;gt;','&amp;quot;'], ['&lt;', '&gt;', '&quot;'], str);
}
...
function parsecode(text1,text) {
	codecount++;
	text = text.replace(/^[\n\r]*([\s\S]+?)[\n\r]*$/ig, '$1');
	text = htmlspecialcharscode(text);
	text = preg_replace(['\t', '   ', '  ',' ', '(\r\n|\n|\r)'], ['&nbsp; &nbsp; &nbsp; &nbsp; ', '&nbsp; &nbsp;', '&nbsp;&nbsp;','&nbsp;','<br/>'], text);
	text1 = isUndefined(text1)?'':text1;
	codehtml[codecount] = '[code'+text1+']' + text + '[/code]';
