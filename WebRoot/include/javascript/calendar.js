...
function loadcalendar() {
	...
	s += '<tr align="center" id="calendar_week"><td><a href="###" onclick="refreshcalendar(yy, mm-1)" title="'+lang['old_month']+'">\u300a</a></td><td colspan="5" style="text-align: center"><a href="###" onclick="showdiv(\'year\');doane(event)" class="dropmenu" title="'+lang['select_year']+'" id="year"></a>&nbsp; - &nbsp;<a id="month" class="dropmenu" title="'+lang['select_month']+'" href="###" onclick="showdiv(\'month\');doane(event)"></a></td><td><A href="###" onclick="refreshcalendar(yy, mm+1)" title="'+lang['next_month']+'">\u300b</A></td></tr>';
	s += '<tr id="calendar_header"><td>'+lang['calendar_Sun']+'</td><td>'+lang['calendar_Mon']+'</td><td>'+lang['calendar_Tue']+'</td><td>'+lang['calendar_Wed']+'</td><td>'+lang['calendar_Thu']+'</td><td>'+lang['calendar_Fri']+'</td><td>'+lang['calendar_Sat']+'</td></tr>';
	for(var i = 0; i < 6; i++) {
		s += '<tr>';
		for(var j = 1; j <= 7; j++)
	...
	s += '<tr id="hourminute"><td colspan="7" align="center"><input type="text" size="2" value="" id="hour" onKeyUp=\'this.value=this.value > 23 ? 23 : zerofill(this.value);controlid.value=controlid.value.replace(/\\d+(\:\\d+)/ig, this.value+"$1")\'> '+lang['calendar_hr']+' <input type="text" size="2" value="" id="minute" onKeyUp=\'this.value=this.value > 59 ? 59 : zerofill(this.value);controlid.value=controlid.value.replace(/(\\d+\:)\\d+/ig, "$1"+this.value)\'> '+lang['calendar_min']+'</td></tr>';
	s += '</table></div></div>';
	...
	for(var k = 1; k <= 12; k++) {
		s += '<a href="###" onclick="refreshcalendar(yy, ' + (k - 1) + ');$(\'calendar_month\').style.display=\'none\'"><span' + (today.getMonth()+1 == k ? ' class="calendar_today"' : '') + ' id="calendar_month_' + k + '">' + k + ( k < 10 ? '&nbsp;' : '') + ' '+lang['calendar_month']+'</span></a><br />';
	}
	s += '</div>';
	if(is_ie && is_ie < 7) {
...
function refreshcalendar(y, m) {
	...
	while(x.getMonth() == mm) {
		...
		if(x.getFullYear() == today.getFullYear() && x.getMonth() == today.getMonth() && x.getDate() == today.getDate()) {
			...
			dd.firstChild.title = lang['calendar_today'];
		}
