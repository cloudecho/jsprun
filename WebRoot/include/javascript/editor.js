...
function newEditor(mode, initialtext) {
	...
	smilies_show(editorid+'smiliesdiv', smcols, wysiwyg, editorid);
...
function initEditor() {
	...
	for(var i = 0; i < buttons.length; i++) {
		if(buttons[i].id.indexOf(editorid + '_cmd_') != -1) {
			buttons[i].href = '###';
			buttons[i].onclick = function(e) {jspruncode(this.id.substr(this.id.lastIndexOf('_cmd_') + 5))};
...
function writeEditorContents(text) {
	if(wysiwyg) {
		...
		if(initialized) {
			...
			if(is_ie){
				editdoc.preserveWhiteSpace='true';
			}
...
function jspruncode(cmd, arg) {
	...
	if(in_array(cmd, ['quote', 'code', 'free', 'hide','javacode', 'jscriptcode','sqlcode','xmlcode','csscode','csharpcode'])) {
		var sel;
		if(is_ie) {
			sel = wysiwyg ? editdoc.selection.createRange() : document.selection.createRange();
			var pos = getCaret();
		...
		var selection = sel ? (wysiwyg ? sel.htmlText : sel.text) : getSel();
		var index = cmd.indexOf('code');
		var opentag = '[' + cmd + ']';
		var closetag = '[/' + cmd + ']';
		if(index>0){
			var temp = cmd.substr(0,index);
			temp = temp==''?'':'='+temp;
			opentag = '[code' + temp + ']';
			closetag = '[/code]';
		}
		...
		if(cmd != 'hide' || !selection) {
			var temp = index>0?'code':cmd;
			str += lang['post_jspruncode_' + temp] + ':<br /><textarea id="' + ctrlid + '_param_1" cols="50" rows="5"></textarea>';
		}
		str += cmd == 'hide' && selection ? '' : '<br />';
		str += cmd == 'hide' ? '<input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_1" checked="checked">'+lang['browse_reply']+'<br /><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_2">'+lang['browse_credits']+' <input type="text" size="3" id="' + ctrlid + '_param_2"> '+lang['display'] : '';
		var div = editorMenu(ctrlid, str);
		...
		if(wysiwyg) {
			...
			var str = lang['tablerows']+' <input type="text" id="' + ctrlid + '_param_rows" size="10" value="2" /> &nbsp; '+lang['tablecols']+' <input type="text" id="' + ctrlid + '_param_columns" size="10" value="2" /><br />'+lang['tablewidth']+' <input type="text" id="' + ctrlid + '_param_width" size="10" value="" /> &nbsp; '+lang['bgcolor']+' <input type="text" id="' + ctrlid + '_param_bgcolor" size="10" />';
			var div = editorMenu(ctrlid, str);
...
function setContext(cmd) {
	...
	fs = fs ? fs : lang['post_jspruncode_fontname'];
