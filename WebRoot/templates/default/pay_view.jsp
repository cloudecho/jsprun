...
<div id="nav"><a href="${settings.indexname}"> ${settings.bbname} </a> &raquo; ${navigation} &raquo; <bean:message key="pay_view"/></div>
<div class="mainbox">
	...
	<table summary="<bean:message key="pay_view"/>" cellspacing="0" cellpadding="0">
		...
		<c:choose>
			<c:when test="${payloglist!=null}">
				<c:forEach items="${payloglist}" var="paylog">
					<tr>
			...
			<c:otherwise>
				<tr><td colspan="3"><bean:message key="pay_nobuyers"/></td></tr>
			</c:otherwise>
		</c:choose>
	</table>
</div>
