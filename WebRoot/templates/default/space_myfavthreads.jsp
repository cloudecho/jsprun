		...
		<tr>
			...
			<td id="main_layout1">
				<table class="module" cellpadding="0" cellspacing="0" border="0">
					<tr>
						<td class="header"> <div class="title"><bean:message key="myfavthreads"/></div> </td>
					</tr>
					<tr>
						<td>
							<div id="module_mythreads">
								<table cellspacing="0" cellpadding="0" width="100%">
									...
									<c:forEach items="${myfavthreads}" var="threads">
										<tr>
										<td class="subject">
						...
						<c:if test="${logpage.totalSize > 10}">
							<div class="p_bar">
								...
								<c:if test="${logpage.totalPage>10 && logpage.currentPage>=4}">
									<a href="space.jsp?action=myfavthreads&uid=${member.uid}&amp;page=1" class="p_pages">1 ...</a>
								</c:if>
								
								<c:if test="${logpage.currentPage != logpage.prePage}">
									<a href="space.jsp?action=myfavthreads&uid=${member.uid}&amp;page=${logpage.prePage}" class="p_redirect">&lsaquo;&lsaquo;</a>
								</c:if>
								<c:choose>
									<c:when test="${logpage.totalPage>10 && logpage.currentPage>=4 && logpage.totalPage-(logpage.currentPage-2)>=10}">
										...
										<c:forEach var="num" begin="${logpage.currentPage-2}" end="${(logpage.currentPage-2)+9}" step="1">
											<c:choose>
												...
												<c:otherwise>
													<a href="space.jsp?action=myfavthreads&uid=${member.uid}&amp;page=${num}" class="p_num">${num}</a>
												</c:otherwise>
											</c:choose>
										</c:forEach>
									</c:when>
								...
								<c:if test="${logpage.currentPage != logpage.nextPage}">
									<a href="space.jsp?action=myfavthreads&uid=${member.uid}&amp;page=${logpage.nextPage}" class="p_redirect">&rsaquo;&rsaquo;</a>
								</c:if>
								
								<c:if test="${logpage.totalPage>10 && (logpage.totalPage-logpage.currentPage)>7}">
									<a href="space.jsp?action=myfavthreads&uid=${member.uid}&amp;page=${logpage.totalPage}" class="p_pages">... ${logpage.totalPage}</a>
								</c:if>
								
								<c:if test="${logpage.totalPage>10}">
									<kbd>
										<input type="text" name="custompage" size="3"  onkeydown="if(event.keyCode==13) {window.location='space.jsp?action=myfavthreads&uid=${member.uid}&amp;page='+this.value; return false;}" />
									</kbd>
								</c:if>
						</c:if>
