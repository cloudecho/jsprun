						...
						<c:if test="${logpage.totalSize > 10}">
							<div class="p_bar">
								...
								<c:if test="${logpage.totalPage>10 && logpage.currentPage>=4}">
									<a href="space.jsp?action=myreplies&uid=${member.uid}&amp;page=1" class="p_pages">1 ...</a>
								</c:if>
								...
								<c:if test="${logpage.currentPage != logpage.nextPage}">
									<a href="space.jsp?action=myreplies&uid=${member.uid}&amp;page=${logpage.nextPage}" class="p_redirect">&rsaquo;&rsaquo;</a>
								</c:if>
								
								<c:if test="${logpage.totalPage>10 && (logpage.totalPage-logpage.currentPage)>7}">
									<a href="space.jsp?action=myreplies&uid=${member.uid}&amp;page=${logpage.totalPage}" class="p_pages">... ${logpage.totalPage}</a>
								</c:if>
								
								<c:if test="${logpage.totalPage>10}">
									<kbd>
									<input type="text" name="custompage" size="3"  onkeydown="if(event.keyCode==13) {window.location='space.jsp?action=myreplies&uid=${member.uid}&amp;page='+this.value; return false;}" />
								</kbd>
								</c:if>
						</c:if>
