...
<div id="nav"><a href="${settings.indexname}"> ${settings.bbname} </a> &raquo; <a href="tag.jsp"><bean:message key="a_post_tag"/></a> &raquo; ${name}</div>
<c:if test="${!empty multi.multipage}"><div class="pages_btns">${multi.multipage}</div></c:if>
<div class="mainbox threadlist">
	<h1><bean:message key="a_post_tag"/>: ${name}</h1>
	<table summary="${name}" cellspacing="0" cellpadding="0">
		...
		<c:forEach items="${dislist}" var="thread"><tr>
			<td class="icon">
			<c:choose>
			...
			<th>
				...
				<a href="viewthread.jsp?tid=${thread.tid}" target="_blank" ${thread.highlight}>${thread.subject}</a>
			...
			<td class="author">
				<cite>
					<c:choose>
						<c:when test="${thread.authorid>0&&thread.author!=''}"><a href="space.jsp?uid=${thread.authorid}">${thread.author}</a></c:when>
						<c:otherwise><c:choose><c:when test="${ismoderator}"><a href="space.jsp?uid=${thread.authorid}"><bean:message key="anonymous"/></a></c:when><c:otherwise><bean:message key="anonymous"/></c:otherwise></c:choose></c:otherwise>
					</c:choose>
				</cite>
				<em><jrun:showTime timeInt="${thread.dateline}" type="${dateformat}" timeoffset="${timeoffset}" replErTime="N/A"/></em>
			</td>
			<td class="nums"><strong>${thread.replies}</strong> / <em>${thread.views}</em></td>
			<td class="lastpost">
				<em><a href="redirect.jsp?tid=${thread.tid}&goto=lastpost&highlight=${index.keywords}#lastpost"><jrun:showTime timeInt="${thread.lastpost}" type="${dateformat} ${timeformat}" timeoffset="${timeoffset}" replErTime="N/A"/></a></em>
				<cite>by <c:choose><c:when test="${thread.lastposter!=''}"><a href="space.jsp?username=<jrun:encoding value="${thread.lastposter}"/>" target="_blank">${thread.lastposter}</a></c:when><c:otherwise><bean:message key="anonymous"/></c:otherwise></c:choose></cite>
			</td>
		</tr></c:forEach>
		<c:if test="${empty dislist}"><tr><td colspan="6"><bean:message key="tag_nofound"/></td></tr></c:if>
	</table>
</div>
<c:if test="${!empty multi.multipage}"><div class="pages_btns">${multi.multipage}</div></c:if>
